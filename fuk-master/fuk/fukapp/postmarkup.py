...
class QuoteTag(TagBase):
    ...
    def render_open(self, parser, node_index):
        if self.params:
            ...
            return u'<div class="quote-msg"><span class="quote-author">%s wrote:</span>'%(PostMarkup.standard_replace(self.params))
        else:
            return u'<div class="quote-msg">'
    ...
    def render_to_html(self,
        ...
        for tag_type, tag_token, start_pos, end_pos in self.tokenize(post_markup):
            ...
            if tag_type == TOKEN_TEXT:
                ...
                #         tag_name = tag_token
                #         tag_attribs = u""
                """ We have to switch the order of token splitting around here, to allow for quotes with '='s and spaces.
                    For instance [quote=Jo User], which is common on fuk. [2010-12-13 3:46:30] by Ben"""
                if '=' in tag_token:
                        tag_name, tag_attribs = tag_token.split(u'=', 1)
                        tag_attribs = tag_attribs.strip()
                else:
